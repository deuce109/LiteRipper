...
module Utils =
    ...
    let generateFileSize (filesize: float) : string =
        let rec loop size iteration =
            match size with
            | check when check < float 1000 -> check, iteration
            | _ -> loop (size / float 1000) (iteration + 1)
    ...
    let rec selectDrives (drives: DriveInfo list, predicate: DriveInfo -> bool) : DriveInfo list =
        match drives with
        | head :: tail when predicate (head) -> head :: selectDrives (tail, predicate)
        | _ :: tail -> selectDrives (tail, predicate)
        | [] -> []
    ...
    let rec printDrives (drives: DriveInfo list) =
        if drives.Length <> 0 then
            ...
            <| generateFileSize (
                float drives.Head.TotalSize
                - float drives.Head.AvailableFreeSpace
            )
    ...
    let getDrivesByType (driveType: string) : DriveInfo list =
        ...
        selectDrives (drives, (fun x -> x.DriveType = driveTypeObj))
